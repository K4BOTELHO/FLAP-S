...
void loop() {
  ...
  if (mySwitch.available()) {
    ...
    if (value == 0) {
      ...
      if (mySwitch.getReceivedValue() == 156057685) {  //Primeiro Botao
        tempobotao = millis();
        delay(200);
      ...
      } else if (mySwitch.getReceivedValue() == 156057701) {  //Segundo Botao
        tempobotao = millis();
        delay(200);
...
void Subir() {
  if (volta == true && encStatus == 1) {  // No centro -> Posicao correta
    ...
    EEPROM.write(17, controleoff);
    controleoff2 = 0;
    EEPROM.write(18, controleoff2);

    ultimoCmdTil = 1;
    EEPROM.write(24, ultimoCmdTil);
    estadoCmdTil = 10;
    EEPROM.write(23, estadoCmdTil);
...
void Direito() {
  if (digitalRead(enablePan) == LOW) {
    if (volta == true) {
      ...
      if (contador == 1) {
        dirStatus = dir;
        EEPROM.write(13, dirStatus);
        esqStatus = esq;
        EEPROM.write(14, esqStatus);
        control = 0;
        EEPROM.write(19, control);
      }
      contador = 0;
      EEPROM.write(16, contador);
      ultimoCmdPan = 1;
      EEPROM.write(21, ultimoCmdPan);
      encStatus = 0;
      EEPROM.write(20, encStatus);
      momentoDir = 1;
      EEPROM.write(25, momentoDir);
